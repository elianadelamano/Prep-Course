...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  [3,345,174,85]
  var numerosMasGande = numeros[0]
  for(var i =1; i< numeros.length; i++ ){
    if(numeros[i] > numerosMasGande ){
      numerosMasGande =numeros[i]
    }
  }
  return numerosMasGande;
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  var resultadosDeLaMultiplicacion = 1;
  for(var i = 0; i <arguments.length; i++){
    resultadosDeLaMultiplicacion *= arguments[i]
  }
  return resultadosDeLaMultiplicacion;
}
...
function cuentoElementos(arreglo){
  ...
  //Escribe tu código aquí
  var contadorDeElentos = 0;
  for(var i = 0; i < arreglo.length; i++){
    if (arreglo[i] > 18) {
      contadorDeElentos++;  
    }
  }
  return contadorDeElentos
} 
